let navbar = () => {
	return `<div class="kadonate">
    <a
        href="https://bank.gov.ua/en/news/all/natsionalniy-bank-vidkriv-spetsrahunok-dlya-zboru-koshtiv-na-potrebi-armiyi"
        target="_blank"
        >Stand with Ukraine 🇺🇦 <u>Donate to support</u></a
    >
</div>
<div id="kacontainer">
    <div class="kamenubar">
        <a href="./home.html"
            ><img
                src="https://mailtrap.io/wp-content/uploads/2021/04/mailtrap-new-logo.svg"
                alt="mailtrap"
        /></a>
        <ul>
            <li>
                <a href="#"
                    >Email Testing<img
                        src="data:image/svg+xml;base64,"
                        alt=""
                        width="10px"
                        style="margin-left: 10px"
                /></a>
                <div class="dropdown_menu">
                    <ul>
                        <li>
                            <a href="#">
                                <div class="kaflex">
                                    <div>
                                        <img
                                            src="https://mailtrap.io/wp-content/uploads/2021/06/qa_automation_icon_menu.svg"
                                            alt=""
                                        />
                                    </div>
                                    <div class="kaside">
                                        <h4>QA Automation</h4>
                                        <p>Automation for Email Testing</p>
                                    </div>
                                </div>
                            </a>
                        </li>
                        <li>
                            <a href="#"
                                ><div class="kaflex">
                                    <div>
                                        <img
                                            src="https://mailtrap.io/wp-content/uploads/2021/06/check_html_icon_menu.svg"
                                            alt=""
                                        />
                                    </div>
                                    <div class="kaside">
                                        <h4>HTML Check</h4>
                                        <p>Find HTML/CSS issues</p>
                                    </div>
                                </div></a
                            >
                        </li>
                        <li>
                            <a href="#"
                                ><div class="kaflex">
                                    <div>
                                        <img
                                            src="https://mailtrap.io/wp-content/uploads/2021/06/fake_smtp_server_icon_menu.svg"
                                            alt=""
                                        />
                                    </div>
                                    <div class="kaside">
                                        <h4>Fake SMTP Server</h4>
                                        <p>Your own STMP server</p>
                                    </div>
                                </div></a
                            >
                        </li>
                        <li>
                            <a href="#"
                                ><div class="kaflex">
                                    <div>
                                        <img
                                            src="https://mailtrap.io/wp-content/uploads/2021/06/api_icon_menu.svg"
                                            alt=""
                                        />
                                    </div>
                                    <div class="kaside">
                                        <h4>Mailtrap API</h4>
                                        <p>Integrate with your application</p>
                                    </div>
                                </div></a
                            >
                        </li>
                    </ul>
                </div>
            </li>
            <li><a href="#">Email API</a></li>
            <li><a href="#">Pricing</a></li>
            <li>
                <a href="#"
                    >Resources<img
                        src="data:image/svg+xml;base64,"
                        alt=""
                        width="10px"
                        style="margin-left: 10px"
                /></a>
                <div class="dropdown_menu">
                    <ul>
                        <li>
                            <a target="_blank" href="https://mailtrap.docs.apiary.io/#"
                                ><div class="kaflex">
                                    <div>
                                        <img
                                            src="https://mailtrap.io/wp-content/uploads/2022/06/Transactional-Email-Sending_icon-09.svg"
                                            alt=""
                                            width="40px"
                                        />
                                    </div>
                                    <div class="kaside">
                                        <h4>API</h4>
                                        <p>Integrate with exsiting resources</p>
                                    </div>
                                </div></a
                            >
                        </li>
                        <li>
                            <a href="#"
                                ><div class="kaflex">
                                    <div>
                                        <img
                                            src="https://mailtrap.io/wp-content/uploads/2022/06/Transactional-Email-Sending_icon-08.svg"
                                            alt=""
                                            width="40px"
                                        />
                                    </div>
                                    <div class="kaside">
                                        <h4>Case Studies</h4>
                                        <p>Success stories of our customers</p>
                                    </div>
                                </div></a
                            >
                        </li>
                        <li>
                            <a href="#"
                                ><div class="kaflex">
                                    <div>
                                        <img
                                            src="https://mailtrap.io/wp-content/uploads/2022/06/Transactional-Email-Sending_icon-07.svg"
                                            alt=""
                                            width="40px"
                                        />
                                    </div>
                                    <div class="kaside">
                                        <h4>Blog</h4>
                                        <p>The best content about emails</p>
                                    </div>
                                </div></a
                            >
                        </li>
                        <li>
                            <a href="#"
                                ><div class="kaflex">
                                    <div>
                                        <img
                                            src="https://mailtrap.io/wp-content/uploads/2022/03/Transactional-Email-Sending_icon-06.svg"
                                            alt=""
                                            width="40px"
                                        />
                                    </div>
                                    <div class="kaside">
                                        <h4>Help</h4>
                                        <p>How-to's and Knowledge base</p>
                                    </div>
                                </div></a
                            >
                        </li>
                    </ul>
                </div>
            </li>
        </ul>
        <div id="kabutton">
            <a href="#"><button class="kalog">Log In</button></a>
            <a href="#"><button class="kasign">Sign Up</button></a>
        </div>
    </div>
</div>`;
};

let silonavbar = () => {
	return `
<div id="kacontainer">
    <div class="kamenubar">
        <a href="./home.html"
            ><img
                src="https://mailtrap.io/wp-content/uploads/2021/04/mailtrap-new-logo.svg"
                alt="mailtrap"
        /></a>
        <div id="kabutton">
            <a href="#"><button class="kalog">Log In</button></a>
            <a href="#"><button class="kasign">Sign Up</button></a>
        </div>
    </div>
</div>`;
};

let footer = () => {
	return `<div class="kafootercontainer">
    <div class="kafooterrow">
        <div class="col-md-3">
            <a
                href="https://railsware.com/"
                target="_blank"
                rel="nofollow"
                aria-label="Footer logo link to home"
                class="main-footer__logo-link"
            >
                <svg
                    xmlns="https://www.w3.org/2000/svg"
                    xmlns:xlink="https://www.w3.org/1999/xlink"
                    width="108"
                    height="104"
                    viewBox="0 0 108 104"
                >
                    <defs>
                        <mask
                            id="a"
                            x="17"
                            y="67.38"
                            width="76.96"
                            height="17.36"
                            maskUnits="userSpaceOnUse"
                        >
                            <path d="M17,69.38H94V84.74H17Z" style="fill: #fff"></path>
                        </mask>
                        <mask
                            id="b"
                            x="17.09"
                            y="34.05"
                            width="74.84"
                            height="26.75"
                            maskUnits="userSpaceOnUse"
                        >
                            <path
                                d="M17.09,34.05H91.92V60.8H17.09Z"
                                style="fill: #fff"
                            ></path>
                        </mask>
                    </defs>
                    <path
                        d="M82.77.5H5A4.51,4.51,0,0,0,.5,5V83L20.21,103.5H103a4.51,4.51,0,0,0,4.5-4.5V25.21Z"
                        style="
                            fill: none;
                            stroke: #202c45;
                            opacity: 0.10000000149011612;
                            isolation: isolate;
                        "
                    ></path>
                    <path
                        d="M18.76,23.08a2.38,2.38,0,0,1-1.1-.24,1.88,1.88,0,0,1-.75-.7,2.16,2.16,0,0,1-.26-1.06V18.61a2.08,2.08,0,0,1,.26-1,1.79,1.79,0,0,1,.75-.69,2.26,2.26,0,0,1,1.1-.25,2.45,2.45,0,0,1,1.11.24,1.76,1.76,0,0,1,.74.69,1.88,1.88,0,0,1,.27,1,.08.08,0,0,1,0,.07l-.06,0-.46,0c-.06,0-.09,0-.09-.08v0a1.34,1.34,0,0,0-.41-1,1.5,1.5,0,0,0-1.07-.38,1.52,1.52,0,0,0-1.07.38,1.35,1.35,0,0,0-.4,1v2.52a1.35,1.35,0,0,0,.4,1,1.48,1.48,0,0,0,1.07.38,1.5,1.5,0,0,0,1.07-.37,1.36,1.36,0,0,0,.41-1v0c0-.05,0-.08.09-.08l.46,0c.06,0,.09,0,.09.08a2,2,0,0,1-.27,1,1.76,1.76,0,0,1-.74.69A2.45,2.45,0,0,1,18.76,23.08ZM26,23a.09.09,0,0,1-.1-.06l-1.33-2.79a0,0,0,0,0,0,0H23s0,0,0,0v2.75a.08.08,0,0,1-.09.09H22.4a.08.08,0,0,1-.09-.09V16.79a.08.08,0,0,1,.09-.09h2.33a1.63,1.63,0,0,1,1.25.49,1.66,1.66,0,0,1,.48,1.25,1.69,1.69,0,0,1-.34,1.07,1.61,1.61,0,0,1-.93.57s0,0,0,0l1.35,2.77a.07.07,0,0,1,0,.05s0,.06-.07.06Zm-3-5.74s0,0,0,0v2.28s0,0,0,0h1.67a1.11,1.11,0,0,0,1.17-1.17,1.18,1.18,0,0,0-.32-.85,1.15,1.15,0,0,0-.85-.33ZM31.89,23a.11.11,0,0,1-.11-.07l-.36-1.14s0,0,0,0H28.7a.05.05,0,0,0,0,0l-.36,1.14a.1.1,0,0,1-.11.07h-.48c-.07,0-.09,0-.07-.1l2-6.13a.11.11,0,0,1,.11-.07h.61a.1.1,0,0,1,.11.07l2,6.13v0s0,.06-.08.06Zm-3.05-1.8v0h2.39a.08.08,0,0,0,0,0l-1.17-3.67,0,0,0,0Zm8.85-4a.08.08,0,0,1-.09.09H34.3s0,0,0,0v2.23s0,0,0,0h2.31a.08.08,0,0,1,.09.09V20a.08.08,0,0,1-.09.09H34.3a0,0,0,0,0,0,0v2.76a.08.08,0,0,1-.09.09h-.46a.08.08,0,0,1-.09-.09V16.79a.08.08,0,0,1,.09-.09H37.6a.08.08,0,0,1,.09.09Zm4.85-.47a.08.08,0,0,1,.09.09v.39a.08.08,0,0,1-.09.09H40.77a0,0,0,0,0,0,0v5.61a.08.08,0,0,1-.09.09h-.46a.08.08,0,0,1-.09-.09V17.3s0,0,0,0H38.37a.08.08,0,0,1-.09-.09v-.39a.08.08,0,0,1,.09-.09Zm5.4.47a.08.08,0,0,1-.09.09H44.54s0,0,0,0v2.23s0,0,0,0h2.32a.08.08,0,0,1,.09.09V20a.08.08,0,0,1-.09.09H44.54s0,0,0,0v2.26s0,0,0,0h3.31a.08.08,0,0,1,.09.09v.38a.08.08,0,0,1-.09.09H44a.08.08,0,0,1-.09-.09V16.79A.08.08,0,0,1,44,16.7h3.9a.08.08,0,0,1,.09.09ZM49.4,23a.08.08,0,0,1-.09-.09V16.79a.08.08,0,0,1,.09-.09h2.13a2.05,2.05,0,0,1,1.41.47,1.61,1.61,0,0,1,.53,1.27v2.82a1.61,1.61,0,0,1-.53,1.27,2.05,2.05,0,0,1-1.41.47Zm.55-.59s0,0,0,0h1.56a1.25,1.25,0,0,0,.93-.34,1.24,1.24,0,0,0,.35-.92V18.53a1.18,1.18,0,0,0-1.28-1.27H50s0,0,0,0Zm9.89-2.69s0,0,0,.05a1.61,1.61,0,0,1,.74.58,1.75,1.75,0,0,1,.25.94,1.6,1.6,0,0,1-.52,1.26A2,2,0,0,1,59,23H56.77a.08.08,0,0,1-.09-.09V16.79a.08.08,0,0,1,.09-.09h2.11a2,2,0,0,1,1.37.43,1.52,1.52,0,0,1,.51,1.22,1.44,1.44,0,0,1-.92,1.37Zm-2.49-2.46a0,0,0,0,0,0,0v2.16s0,0,0,0h1.53a1.29,1.29,0,0,0,.9-.3,1,1,0,0,0,.35-.79,1.06,1.06,0,0,0-.35-.83,1.25,1.25,0,0,0-.9-.31ZM59,22.43a1.24,1.24,0,0,0,.9-.32,1.18,1.18,0,0,0,.34-.87,1.16,1.16,0,0,0-.35-.88,1.29,1.29,0,0,0-.93-.33H57.35s0,0,0,0V22.4a0,0,0,0,0,0,0Zm4.7.56a.08.08,0,0,1-.09-.09V20.24a.2.2,0,0,0,0,0l-1.87-3.38a.08.08,0,0,1,0-.06s0-.05.08-.05h.49a.09.09,0,0,1,.1.06l1.52,2.75h0l1.52-2.75a.11.11,0,0,1,.11-.06H66a.08.08,0,0,1,.08,0s0,0,0,.07l-1.87,3.38s0,0,0,0V22.9a.08.08,0,0,1-.09.09Z"
                        style="fill: #1a2e44"
                    ></path>
                    <g style="mask: url(#a)">
                        <path
                            d="M90.65,75.67c1.67-.64,1.43-2.34-.64-2.57-.22,0-.44,0-.67,0-1.17,0-2.6.36-2.6,1.77v1.64C88.15,75.84,89.4,76.15,90.65,75.67ZM69.16,78.08a1.26,1.26,0,0,0,0,2c1,.77,4.58.77,4.58-1.24v-2a16.39,16.39,0,0,1-3.37.71A2.67,2.67,0,0,0,69.16,78.08Zm0-2.42a7.41,7.41,0,0,1,1.78-.36A10.22,10.22,0,0,0,72.78,75c.94-.28.92-1,.3-1.42a3.4,3.4,0,0,0-1.65-.39H67.61V71h3.71c2.58,0,4.67,1.21,4.67,4v3.35c0,1.32.09,2.2-1,3.19a5.91,5.91,0,0,1-4,1.25,5.75,5.75,0,0,1-3-.81,3.11,3.11,0,0,1-1.4-2.09,3.54,3.54,0,0,1,1.2-3.43A4.6,4.6,0,0,1,69.11,75.66ZM83.77,71h-.94a5.16,5.16,0,0,0-5.16,5.15v6.4h2.24v-6.4a2.93,2.93,0,0,1,2.92-2.92h.94ZM59,74.66l2.17,7.91h2.28L66.72,71H64.48l-2.21,8.08L60.07,71,59,71l-1.11,0-2.2,8.08L53.45,71H51.21l3.31,11.6H56.8ZM38.29,67.38V82.57h2.23V69c0-.94-.16-1.61-1.41-1.61ZM23.1,71h-1A5.16,5.16,0,0,0,17,76.17v6.4h2.23v-6.4a2.94,2.94,0,0,1,2.92-2.92h1Zm2.34,7.19a1.28,1.28,0,0,0,.17,1.88c.95.78,4.61.69,4.61-1.32v-2a16.51,16.51,0,0,1-3.43.71A2.43,2.43,0,0,0,25.44,78.21Zm-1-2a6.46,6.46,0,0,1,2.92-.89A10.22,10.22,0,0,0,29.25,75c.94-.28.93-1,.3-1.42a3.4,3.4,0,0,0-1.65-.39H24.48V71h3.31c2.59,0,4.67,1.21,4.67,4v3.35c0,1.32.09,2.2-1,3.19a6,6,0,0,1-4,1.25c-1.62,0-3.64-.52-4.29-2.19a3.64,3.64,0,0,1-.24-1.36A3.41,3.41,0,0,1,24.44,76.19Zm9.78-8.81V69.6h2.23V69c0-.94-.16-1.61-1.41-1.61Zm0,3.59v11.6h2.23v-10c0-.93-.16-1.61-1.41-1.61ZM47,82.57h-5V80.33h6a1.11,1.11,0,0,0,.71-.26A1,1,0,0,0,49,78.61a1.45,1.45,0,0,0-.57-.37,11.14,11.14,0,0,0-1.68-.41l-.92-.2a4.81,4.81,0,0,1-3.1-1.57A3.11,3.11,0,0,1,43,72a3.28,3.28,0,0,1,2.37-1h4.91V73.2H45.38a1,1,0,0,0-.86,1.63,2.48,2.48,0,0,0,1.42.66l1.6.36c1.4.31,2.66.38,3.5,1.93a3.25,3.25,0,0,1-1.76,4.59,3.52,3.52,0,0,1-.64.16A12.32,12.32,0,0,1,47,82.57ZM93.79,75.5a3.13,3.13,0,0,1-.38.72A5.09,5.09,0,0,1,89.61,78c-2,.18-2.74.43-2.77,1.14s.79,1.19,2.23,1.19h4.1v2.24h-4c-2.34,0-4.67-1.3-4.67-3.43V75.25c0-1.32-.09-2.2,1-3.2a5.66,5.66,0,0,1,3.65-1.25,5.37,5.37,0,0,1,4.23,1.62,2.55,2.55,0,0,1,.44.87A4.46,4.46,0,0,1,93.79,75.5Z"
                            style="fill: #1a2e44; fill-rule: evenodd"
                        ></path>
                    </g>
                    <g style="mask: url(#b)">
                        <path
                            d="M17.09,34.54V46L31.37,60.33h11.5Zm13.08-.49H18.67L33.46,48.84a4.07,4.07,0,0,0,5.75-5.75Zm13.59,0H32.26L57.82,59.62a4.07,4.07,0,0,0,5.75-5.75Zm13.58,0H45.84L71.41,59.62a4.07,4.07,0,0,0,5.75-5.75Zm13.58,0L90.74,53.87A4.07,4.07,0,0,1,85,59.62L59.42,34.05ZM17.09,48.13l12.2,12.2H17.09Z"
                            style="fill: #53b275; fill-rule: evenodd"
                        ></path>
                    </g>
                </svg>
            </a>
        </div>
        <div>
            <h3 style="color: rgb(26, 46, 68); margin-bottom: 8px">Mailtrap</h3>
            <ul>
                <li><a href="#">Pricing</a></li>
                <li><a href="#">Changelog</a></li>
                <li><a href="#">Status</a></li>
                <li><a href="#">Terms of Service</a></li>
                <li><a href="#">Privacy Policy</a></li>
                <li><a href="#">Navigational Information</a></li>
                <li><a href="#">DPA</a></li>
            </ul>
        </div>
        <div>
            <h3 style="color: rgb(26, 46, 68); margin-bottom: 8px">
                Railsware Experience
            </h3>
            <ul>
                <li><a href="#">Mailtrap Blog</a></li>
                <li><a href="#">Coupler.io</a></li>
                <li><a href="#">Smart Checklist for Jira</a></li>
                <li><a href="#">Careers - Railsware Hiring</a></li>
            </ul>
        </div>
        <div>
            <h3 style="color: rgb(26, 46, 68); margin-bottom: 8px">Contact</h3>
            <p class="kacolspan">email: <span>support@mailtrap.io</span></p>
            <div class="kafooterimg">
                <ul>
                    <li><a href="#"><img width="30px" src="https://www.capalino.com/wp-content/uploads/2015/05/facebook-4096-black.png" alt=""></a></li>
                    <li><a href="#"><img width="30px" src="https://findicons.com/files/icons/2779/simple_icons/4096/twitter_4096_black.png" alt=""></a></li>
                    <li><a href="#"><img width="30px" src="https://www.shareicon.net/data/2016/07/13/606885_linkedin_2048x2048.png" alt=""></a></li>
                </ul>
            </div>
            <div>
                <p class="kacolspan">© Railsware Products Studio, Inc.</p>
            </div>
        </div>
    </div>
</div>`;
};

let trust = () => {
	return `<h1>Trusted by more than 600k devs, QAs and mangers worldwide</h1>

    <div class="kaon">
        <img
            src="https://mailtrap.io/wp-content/uploads/2021/04/Adobe.svg"
            alt=""
        />
        <img
            src="https://mailtrap.io/wp-content/uploads/2021/04/Atlassian.svg"
            alt=""
        />
        <img
            src="https://mailtrap.io/wp-content/uploads/2021/04/Calendly.svg"
            alt=""
        />
        <img
            src="https://mailtrap.io/wp-content/uploads/2021/04/Yelp.svg"
            alt=""
        />
        <img
            src="https://mailtrap.io/wp-content/uploads/2021/04/UserTesting.svg"
            alt=""
        />
    </div>`;
};

let boximg = () => {
    return `<div class="ka_contain">
    <h2>Create Your Free Account</h2>
    <h1>In 3 Clicks</h1>

    <div>
        <a href="#"><button>Sign Up Now</button></a>
    </div>
</div>
<div>
    <img
        src="https://mailtrap.io/wp-content/uploads/2021/01/img__cta.svg"
        alt=""
    />
</div>`
}

export { navbar, footer, trust, boximg, silonavbar };
